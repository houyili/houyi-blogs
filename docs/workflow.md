...
```bash
npm run import:draft -- /path/to/paper-repo/blog_body.md --config scripts/fixtures/moe-equal-resources-p1.import.yaml
```
...
The installer symlinks `skills/paper-blog-importer` into `${CODEX_HOME:-~/.codex}/skills`. On another Mac, clone this repo, run `npm install`, then run the install command above. Existing symlinks from older clones are automatically relinked; real files/directories are left untouched. Verify with:
