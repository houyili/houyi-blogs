...
这两种做法都有价值，但它们都没有把 total parameters 锁住。这个变量不是纸面数字，它对应 capacity、HBM footprint、checkpoint size 和最小部署单元；真实系统里，当 batch size 足够大时，几乎所有 experts 都会被激活。因此更硬的问题是：如果 Dense 和 MoE 的 total parameter count 完全相同，MoE 还能不能追平甚至超过 Dense？如果可以，就说明收益不只是 parameter补贴，而是 sparsity 在训练得当时确实能成为架构优势。
